"""
Statistical analysis module for compression experiments.

Provides:
- Paired t-tests between compression formats
...
class StatisticalAnalyzer:
    ...
    def independent_t_test(
        ...
        Note: We use independent t-test (not paired) because the observations
        come from different training runs, not from the same images.
    ...
    def wilcoxon_test(
        ...
        n = len(data1)
        r = statistic / (n * (n + 1) / 2)  # Normalized statistic
...
def analyze_format_comparison(
    ...
    for i, fmt1 in enumerate(format_list):
        for fmt2 in format_list[i + 1:]:
            # Independent t-test (Welch's) - appropriate for different training runs
            ...
            if len(groups[fmt1]) > 0 and len(groups[fmt2]) > 0:
                mw_statistic, mw_p_value = stats.mannwhitneyu(
                    groups[fmt1], groups[fmt2], alternative='two-sided'
                )
                results['pairwise_wilcoxon'].append({
                    'test': 'mann_whitney_u',
                    'comparison': f"{fmt1} vs {fmt2}",
                    'format1': fmt1,
                    'format2': fmt2,
                ...
                p_values_wilcoxon.append(mw_p_value)
...
def generate_statistical_report(
    ...
    for fmt, stats in results.get('descriptive_stats', {}).items():
        lines.append(f"\n{fmt.upper()}:")
    ...
    if results.get('pairwise_tests'):
        lines.append("\n" + "-" * 40)
        lines.append("PAIRWISE COMPARISONS (Independent t-test, Welch's)")
        lines.append("-" * 40)
    ...
    if results.get('pairwise_wilcoxon'):
        lines.append("\n" + "-" * 40)
        lines.append("PAIRWISE COMPARISONS (Mann-Whitney U, Non-parametric)")
        lines.append("-" * 40)
...
def run_full_statistical_analysis(
    ...
    for metric in metrics:
        ...
        results = analyze_format_comparison(df, metric_column=metric)
